...
/* ---------- 갈래 ----------
   왜 같은 주제가 앞뒤로 흩어져 있나 — 커리큘럼을 **여러 번에 나눠 만들었고**
   나중 것을 뒤에 이어 붙였기 때문이다(일상 1~22 → 7·15 끼워넣기 → 23~32 → 33~42).
   그렇다고 지금 번호를 뒤섞을 수는 없다. 두 가지가 걸린다.
     ① 이미 공부한 사람의 진도는 날짜 번호로 저장돼 있다 — 번호를 옮기면 남의 기록이 어긋난다.
     ② 문장은 '그날까지 배운 단어로만' 쓰도록 조립기가 검증한다. 순서를 바꾸면 그 보장이 깨진다.
   덧붙여, 같은 주제를 몰아서 하는 것보다 **띄엄띄엄 다시 만나는 편이 기억에 낫다**(분산 학습).
   그래서 순서는 두고, **갈래로 모아 보는 길을 따로 낸다** — 흩어져 있어도 찾을 수 있게. */
const KIN = {
  '인사와 호칭': ['일상 1', '일상 2', '일상 3', '일상 4', '일상 5', '일상 6'],
  '일과 직업': ['일상 7', '일상 12'],
  '숫자와 돈': ['일상 8', '일상 9', '일상 14', '일상 15'],
  '때와 날짜': ['일상 10', '일상 11', '일상 21', '일상 27'],
  '먹고 마시기': ['일상 13', '일상 30', '일상 38', '일상 39'],
  '길과 주소': ['일상 16', '일상 24', '일상 42'],
  '가족과 고향': ['일상 17', '일상 33', '일상 34'],
  '몸과 병': ['일상 18', '일상 29'],
  '부탁 · 약속 · 평가': ['일상 19', '일상 20', '일상 22'],
  '날씨와 색': ['일상 23', '일상 25'],
  '마음과 맞장구': ['일상 26', '일상 36', '일상 37'],
  '생활 살림': ['일상 28', '일상 32', '일상 40', '일상 41'],
  '명절과 스포츠': ['일상 31', '일상 35'],
  '회사와 사람들': ['직무 1', '직무 26', '직무 31', '직무 32', '직무 33'],
  '식당과 기숙사': ['직무 8', '직무 11', '직무 25'],
  '숫자 · 수량 · 납기': ['직무 2', '직무 7', '직무 9', '직무 29'],
  '지시 · 보고 · 관리': ['직무 4', '직무 6', '직무 41', '직무 42', '직무 43', '직무 44', '직무 45'],
  '안전과 환경': ['직무 3', '직무 23', '직무 46', '직무 59'],
  '기계와 전기': ['직무 5', '직무 12', '직무 57'],
  '공장 안에서': ['직무 10', '직무 51', '직무 52', '직무 55'],
  '봉제 라인': ['직무 13', '직무 14', '직무 16', '직무 17', '직무 18', '직무 20', '직무 54'],
  '전자 라인': ['직무 21', '직무 22', '직무 56'],
  '품질과 불량': ['직무 15', '직무 19', '직무 24', '직무 53', '직무 58', '직무 60'],
  '손님과 소통': ['직무 27', '직무 28', '직무 35'],
  '서류와 제도': ['직무 30', '직무 34', '직무 36', '직무 37', '직무 38'],
  '몸과 사고': ['직무 39', '직무 40'],
  '물류와 창고': ['직무 47', '직무 48', '직무 49', '직무 50'],
};
const setKey = d => (d.track === 'work' ? '직무 ' : '일상 ') + (d.n || d.day);
const KINOF = {};
Object.entries(KIN).forEach(([g, ks]) => ks.forEach(k => { KINOF[k] = g; }));
const dayByKey = k => ALL.find(d => typeof d.day === 'number' && setKey(d) === k);
/* 이 세트가 속한 갈래와, 같은 갈래의 다른 세트들 */
function kinOf(d) {
  const g = KINOF[setKey(d)];
  if (!g) return null;
  const others = KIN[g].filter(k => k !== setKey(d)).map(dayByKey)
    .filter(x => x && visibleDay(x));
  return others.length ? { name: g, others: others.map(x => ({ d: x })) } : null;
}
...
function renderDays(track) {
  ...
  // 같은 주제가 앞뒤로 흩어져 있어도 한자리에 모아 볼 수 있게
  const sw = el('li', 'catpick');
  sw.append(el('span', null, '보기 '));
  [['날짜순', 0], ['갈래로', 1]].forEach(([nm2, v]) => {
    const on = (S.bykin ? 1 : 0) === v;
    const bb = el('button', 'ghost sm' + (on ? ' pick' : ''), (on ? '✓ ' : '') + nm2);
    bb.onclick = () => { S.bykin = v; save(); renderDays(track); };
    sw.append(bb);
  });
  list.append(sw);
  ...
  if (S.bykin) {                            // 갈래로 — 주제가 같은 것끼리 모아 놓는다
    const seen = new Set();
    Object.entries(KIN).forEach(([gname, keys]) => {
      const got = keys.map(k => days.find(d => setKey(d) === k)).filter(Boolean);
      if (!got.length) return;
      list.append(el('li', 'grp', esc(gname)));
      got.forEach(d => { seen.add(d.day); list.append(row(d)); });
    });
    const rest = days.filter(d => !seen.has(d.day));
    if (rest.length) {
      list.append(el('li', 'grp', '그 밖에'));
      rest.forEach(d => list.append(row(d)));
    }
...
function drawCard() {
  ...
  if (it.k === 'cover') {
    ...
    if (x.kin) {                              // 같은 갈래의 다른 세트 — 흩어져 있어도 여기서 이어 간다
      const k = el('div', 'kinbox');
      k.append(el('div', 'kint', '같은 갈래 · ' + esc(x.kin.name)));
      x.kin.others.forEach(o => {
        const bb = el('button', 'ghost sm' + (S.done[o.d.day] ? ' pick' : ''),
                      esc(label(o.d) + ' ' + o.d.theme));
        bb.onclick = () => startLearn(o.d);
        k.append(bb);
      });
      c.append(k);
    }
...
/* 대화창의 성조 줄 — 입력칸 바로 위에 늘 붙어 있는 얇은 한 줄.
   폰 자신의 자판으로 한글도 베트남어도 치고, 이 줄로 부호만 얹는다.
   (예전에는 우리 자판을 통째로 띄웠는데, 그러면 폰 자판이 막혀 **한글을 못 쳤다**.)
   · 성조 여섯 — 마지막 낱말의 주모음에 얹는다
   · ă â ê ô ơ ư đ — 방금 친 글자가 그 밑글자면 바꿔치고, 아니면 그냥 붙인다
     (com 치고 ơ 를 누르면 cơm 이 된다) */
const TONEROW = [['', 'ngang'], ['\u0300', 'huyền'], ['\u0301', 'sắc'],
                 ['\u0309', 'hỏi'], ['\u0303', 'ngã'], ['\u0323', 'nặng']];
/* 'ơ' 도 'ắ' 도 결국 밑글자는 a·o 다. 부호를 두 겹(모자+성조) 벗겨 밑글자만 남긴다. */
...
function drawChatTone() {
  ...
  const key = (label, fn, cls) => {
    ...
    k.type = 'button';
    k.addEventListener('mousedown', e => e.preventDefault());   // 눌러도 자판이 안 내려가게
    k.onclick = () => { fn(); chatGrow(); inp.focus({ preventScroll: true }); };
    return k;
  };
  TONEROW.forEach(([mk, name]) => bar.append(key(toneArrow(name), () => {
  ...
  LETROW.forEach(([ch, base]) => bar.append(key(ch, () => {
    // 마지막 낱말 안에서 그 밑글자를 **뒤에서부터** 찾아 바꾼다 (com + ơ → cơm).
    // 실제 베트남 자판(텔렉스)도 이렇게 움직인다 — 글자를 다 치고 부호를 얹는다.
    const v = inp.value, cut = v.lastIndexOf(' ') + 1, word = v.slice(cut);
    for (let i = word.length - 1; i >= 0; i--) {
      if (plainLetter(word[i]) === base)
        { inp.value = v.slice(0, cut) + word.slice(0, i) + ch + word.slice(i + 1); return; }
    }
    inp.value = v + ch;
  })));
}
...
$('#chatLog').addEventListener('pointerdown', e => {
  if (!e.target.closest('button, a, input, textarea')) $('#chatText').blur();
});
...
const CULTURE = [
  ...
  { e: '⚽', t: '축구가 국민 스포츠', b: '한 조사에서 <b>열 명 중 일곱</b>이 축구 팬이라고 답했습니다. 국가대표 경기가 있는 날은 거리가 붉은 깃발로 덮이고, 이기면 오토바이 행렬이 밤새 경적을 울립니다.<br>' +
      '축구 이야기는 어디서나 통하는 말문 트기입니다. 다만 <b>어느 팀을 응원하느냐</b>보다 <b>대표팀 경기</b> 이야기가 안전합니다.' },
  ...
  { e: '🛵', t: '그랩이 택시이자 배달', b: '<b>Grab</b>(그랩) 앱 하나로 오토바이 택시·자동차 택시·음식 배달이 다 됩니다. <b>ShopeeFood</b>·<b>Be</b> 도 함께 씁니다.<br>' +
      '값이 앱에 미리 뜨고 지도로 따라오니 <b>말이 안 통해도 탈 수 있습니다</b> — 베트남에 도착해 가장 먼저 도움이 되는 앱입니다.<br>' +
      '오토바이 택시는 <b>헬멧을 기사가 줍니다</b>. 헬멧은 법으로 의무입니다.' },
  ...
  { e: '🚰', t: '마시는 물은 사서 마신다', b: '수돗물을 그대로 마시지 않습니다. <b>끓여 마시거나 생수를 삽니다.</b><br>' +
      '집·사무실에는 <b>20리터 생수통(bình nước)</b>을 배달시켜 정수기 받침에 올려 씁니다 — 통을 다 쓰면 전화 한 통에 갈아 줍니다.<br>' +
      '식당의 차(<b>trà đá</b> 짜 다 — 얼음 녹차)는 대개 공짜로 나옵니다.' },
...
/* 세트 → 그 자리에 어울리는 문화 이야기. **번호가 아니라 이름으로** 짝짓는다
   (번호로 하면 카드를 하나 끼워 넣을 때마다 짝이 통째로 밀린다 — 실제로 한 번 밀렸다).
   억지로 채우지 않는다. 안 맞는 자리는 비워 둔다 — 표지와 딴소리가 나면 안 하느니만 못하다.
   정말 맞는 자리라면 한 장을 두 곳까지 쓴다. 세 곳부터는 쓰지 않는다. */
const CULTAT = {
  // ── 일상 ──
  '일상 1': '호칭이 예의의 절반',
  '일상 2': '이름은 뒤에서 부른다',
  ...
  '직무 3': '안전은 서류가 아니라 습관',
  '직무 5': '220V, 플러그는 그대로',
  '직무 6': '연차와 잔업에는 한도가 있다',
  '직무 7': '날짜는 일 / 월 / 년',
  '직무 8': '한국 라면을 이미 먹고 있다',
  '직무 9': '월급에서 빠지는 것',
  '직무 11': '잘로가 여기의 카톡',
  '직무 26': '가게 앞의 작은 제단',
  '직무 27': '차부터 한 잔',
  ...
  '직무 32': '하지 않는 것이 좋은 일',
  '직무 33': '회식과 건배',
  '직무 34': '설(Tết)이 일 년의 중심',
  '직무 36': '최저임금은 지역마다 다르다',
  '직무 38': '노동허가서가 먼저다',
  '직무 40': '도착한 첫 주에 할 일',
  '직무 43': '지적은 따로, 칭찬은 여럿 앞에서',
};
const CULTBY = {};
CULTURE.forEach(c => { CULTBY[c.t] = c; });
const cultureFor = d => CULTBY[CULTAT[(trackName(d) || '') + label(d).replace('Day ', '')]
                             || CULTAT[(d.track === 'work' ? '직무 ' : '일상 ') + (d.n || d.day)]] || null;
