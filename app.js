...
async function showTone(text, blobUrl, box) {
  ...
  if (j.v === 'ok') {
    ...
    // hỏi·ngã — 원어민도 갈리는 성조. 틀렸다고 하지 않고 사실을 알려 준다.
    verdict(host, 1, null, '높낮이', `${j.wantKo}이어야 합니다`);
    host.append(el('div', 'fixtip',
      '↳ 이 두 성조(<b>hỏi</b> 와 <b>ngã</b>)는 <b>남부 베트남에서 아예 하나로 합쳐졌습니다</b> — ' +
      '남부는 실질적으로 다섯 성조이고, 남부·중부 사람이 맞춤법에서 가장 많이 틀리는 것도 이 둘입니다.<br>' +
      '<b>틀렸다고 하지 않겠습니다.</b> 북부 소리로는 <b>내렸다가 다시 올립니다</b> — 위 곡선을 눈으로 견줘 보세요.'));
  } else {
...
function drawChatTone() {
  ...
  bar.dataset.on = '1';
  const inp = $('#chatText');
  /* 한글 ↔ 베트남어 — 실제 자판의 한/영 키처럼.
     솔직히: **폰 자판 자체의 언어는 웹 화면이 바꾸지 못합니다.** 그건 자판의 지구본 키가 합니다.
     여기서 하는 일은 ① 성조 글쇠를 켜고 끄고 ② 자판에 어느 말인지 알려 주는 것(lang)입니다.
     그래도 '지금 무슨 말을 치는 중인지'가 눈에 보여서 헷갈리지 않습니다. */
  const lang = el('button', 'tk lang vi', 'ă');
  lang.type = 'button';
  lang.addEventListener('mousedown', e => e.preventDefault());
  const setLang = vi => {
    S.chatvi = vi ? 1 : 0; save();
    lang.textContent = vi ? 'ă' : '가';
    lang.title = vi ? '베트남어 — 눌러서 한글로' : '한글 — 눌러서 베트남어로';
    lang.classList.toggle('vi', vi);
    inp.setAttribute('lang', vi ? 'vi' : 'ko');
    inp.placeholder = vi ? 'Tiếng Việt…' : '한국어로 써도 됩니다…';
    bar.querySelectorAll('.tk:not(.lang)').forEach(k => { k.hidden = !vi; });
    if (!vi && !S.langtip) {                        // 딱 한 번만 알려 준다
      S.langtip = 1; save();
      popup('<b>폰 자판의 언어는 자판이 바꿉니다</b><br>' +
            '자판 아래 <b>🌐 지구본</b> 키(안드로이드) 또는 <b>띄어쓰기 옆 지구본</b>(아이폰)을 누르세요.<br>' +
            '이 단추는 <b>성조 글쇠를 켜고 끄는 것</b>입니다.');
    }
  };
  lang.onclick = () => setLang(!(S.chatvi !== 0));
  bar.append(lang);
...
function showGuide() {
  ...
  sec('④', '카드 한 장 안의 모든 것', [
    ...
    '<b>기계는 완벽하지 않습니다.</b> 높낮이를 가리는 정확도는 <b>10에 8~9</b>입니다. 그래서 <b>확실할 때만 O나 X를 냅니다</b> — ' +
      '애매하면 "가려내기 어렵습니다"라고 하고 <b>틀렸다고 하지 않습니다.</b>',
    '<b>hỏi와 ngã는 아예 틀렸다고 하지 않습니다.</b> 이 둘은 <b>남부에서 하나로 합쳐져</b> 원어민도 안 가리고, ' +
      '기계도 여기서 가장 많이 틀렸습니다(잘못된 지적 17.8% — 다른 성조는 0.8~3.3%). 곡선만 보여드립니다.',
    '<b>손글씨는 AI가 채점합니다.</b> 읽어내는 것이 아니라 <b>정답 글씨와 나란히 놓고 견줍니다</b> — ' +
      '글자·성조·모자를 따로 봅니다. 확신이 없으면 점수를 매기지 않고, 틀렸다고 해도 되돌릴 수 있습니다.',
