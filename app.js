...
async function aiListen(text, blobUrl, box) {
  ...
  try {
    ...
    note.innerHTML = (exact || close
      ...
      : '<b>다르게 들립니다</b>') +
      '<span>' + (exact || close
        ? '글자로는 <b>' + esc(stripTone(heard)) + '</b> 로 들렸습니다. 높낮이는 아래 곡선이 봅니다.'
        : '글자로는 <b>' + esc(stripTone(heard)) + '</b> 로 들렸습니다 (목표 <b>' + esc(stripTone(text))
          + '</b>) — 조금 크게, 또박또박') + '</span>';
...
const GROUPS = [
  // 일상 — 표시 번호(n) 기준. 차례를 바꾸면 여기도 같이 바꾼다.
  [d => !d.track && d.n <= 6,               '만나고 인사하기'],
  [d => !d.track && d.n <= 8,               '먼저 급한 말 (먹기 · 아플 때)'],
  [d => !d.track && d.n <= 12,              '숫자와 돈'],
  [d => !d.track && d.n <= 13,              '길 찾기'],
  [d => !d.track && d.n <= 16,              '때와 날짜'],
  [d => !d.track && d.n <= 18,              '일과 직업'],
  [d => !d.track && d.n <= 21,              '부탁 · 평가 · 약속'],
  [d => !d.track && d.n <= 22,              '가족'],
  [d => !d.track && d.n <= 24,              '날씨와 색'],
  [d => !d.track && d.n <= 26,              '쉬는 날 · 아플 때 (더 자세히)'],
  [d => !d.track && d.n <= 29,              '먹고 마시기 (더 자세히)'],
  [d => !d.track && d.n <= 31,              '가족과 고향'],
  [d => !d.track && d.n <= 33,              '길과 탈것'],
  [d => !d.track && d.n <= 36,              '마음과 맞장구'],
  [d => !d.track && d.n <= 40,              '생활 살림'],
  [d => !d.track,                           '명절과 스포츠'],
...
/* 세트 → 그 자리에 어울리는 문화 이야기. **번호가 아니라 주제 이름으로** 짝짓는다.
   ...
   억지로 채우지 않는다. 안 맞는 자리는 비워 둔다. 한 장은 두 곳까지만 쓴다. */
const CULTAT = {
  '일상 — 인사와 호칭': '호칭이 예의의 절반',
  '일상 — 이름 묻고 답하기': '이름은 뒤에서 부른다',
  '일상 — 어느 나라, 어디 사세요': '남과 북은 말이 다르다',
  ...
  '일상 — 타고 다니기': '오토바이가 다리',
  '일상 — 색깔': '붉은색과 흰색',
  '일상 — 마음 말하기': '존댓말 대신 호칭과 ạ',
  '일상 — 쉬는 날': '쉬는 날',
  '일상 — 집안일': '마시는 물은 사서 마신다',
  '일상 — 만나서 한잔': '회식과 건배',
  '일상 — 축하와 명절': '설 첫날에 하지 않는 것',
  '일상 — 베트남에서 살기': '도착한 첫 주에 할 일',
  '일상 — 고향과 나이': '고향(quê)을 묻는다',
  '일상 — 축구 이야기': '축구가 국민 스포츠',
  '일상 — 주말 이야기': '주말에 뭐 하나',
  '일상 — 맞장구와 리액션': '못 알아들었다고 말해도 된다',
  '일상 — 카페': '커피의 나라',
  '일상 — 쌀국수 주문 심화': '먼저 권하고 먹는다',
  '일상 — 유심과 휴대폰': '폰에 뭐가 깔려 있나',
  '일상 — 세탁소와 수선집': '현금 대신 QR',
  '일상 — 택배와 그랩': '그랩이 택시이자 배달',
