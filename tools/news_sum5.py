#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""기사 본문을 **설명하듯 열 줄**로 풀어 준다 → data/news_days.json 의 sum5 칸
...
2026-09-15: 6줄→10줄로 다시 늘렸다(대표님 지시). 줄 수 자체는 근거 조사 범위 밖이라
  논문 근거는 없음 — docs/기준.md 3장 참고.
...
ASK = (
    ...
    "아래 기사를 읽고, 읽는 사람에게 **말로 풀어 주듯** 열 줄로 알려 줘라.\n"
    ...
    "규칙\n"
    " ④ 정확히 10줄. 한 줄은 **28자 안팎**으로 짧게. 길면 카드에서 두 줄로 접혀 답답해진다\n"
    " ⑤ 숫자·회사 이름·지역은 살린다. 다만 한 줄에 숫자를 두 개 넘게 넣지 마라\n"
    " ⑥ 첫 줄은 무슨 일이 있었는지, 마지막 줄은 **우리에게 무슨 뜻인지**.\n"
    "    다만 '우리도' 로 습관처럼 시작하지 마라 — 카드마다 다 똑같아 보인다\n"
    "    (대표님 지적 2026-09-17). 기사 내용에 맞게 자연스럽게 다르게 써라\n"
    " ⑦ 같은 말을 되풀이하지 마라\n"
    " ⑧ **한자를 쓰지 마라.** 반드시 한글로만 쓴다 (예: 正式 대신 정식, 現代 대신 현대)\n"
    ' 출력은 JSON 만: {"sum5": ["줄1","줄2","줄3","줄4","줄5","줄6","줄7","줄8","줄9","줄10"]}\n\n'
)
...
# 2026-09-17: 프롬프트로 "한자 쓰지 마라"·재질문으로도 안 잡히는 경우가 있었다
# (실측: 正式 이 네 번 다 섞여 나와 재시도가 다 소진됨 — 대표님 지적으로 발견).
# 흔히 나오는 것만이라도 직접 바꿔 둔다. 완전한 한자 사전은 아니고 안전망이다.
HANJA_FIX = {
    '正式': '정식', '現代': '현대', '對象': '대상', '政府': '정부', '會議': '회의',
    '計劃': '계획', '計画': '계획', '市場': '시장', '經濟': '경제', '中國': '중국',
    '韓國': '한국', '越南': '베트남', '企業': '기업', '投資': '투자',
}
    ...
    for k in range(tries):
        ...
        if m:
            try:
                ...
                if isinstance(got, list) and len(got) >= 7:  # 2026-09-15: 4→7 (10줄 기준으로 맞춤)
                    out = [tidy(x) for x in got[:10] if str(x).strip()]
                    # 2026-09-17: 프롬프트에 "한자 쓰지 마라"를 넣었지만 모델이 가끔
                    # 어긴다(실측: 正式 이 섞여 나옴, 대표님 지적). 한자가 하나라도
                    # 있으면 그냥 통과시키지 않고 **다시 묻는다**.
                    if any(re.search(r'[一-鿿]', x) for x in out):
                        continue
                    return out
            except Exception:
                pass
