...
CALLS = [
    # 이름,               프롬프트 토큰, 오디오 초, 출력 상한, 크레딧을 깎는가
...
def main():
    ...
    L.append("## ② 쓰는 크레딧 — 실제 API 원가에서 역산")
    L.append("")
    ...
    L.append("")
    L.append("| 호출 | 프롬프트 | 소리 | 출력(상한) | 한 번 원가 | 원화 | 크레딧 깎나 |")
    L.append("|---|---:|---:|---:|---:|---:|---|")
    paid = []
    ...
    top = max(paid)
    L.append("")
    L.append(f"**크레딧을 깎는 호출 가운데 가장 비싼 것이 {top * KRW:.2f}원**이다(쓰기 채점).")
    L.append("")
    L.append("### 그래서 5크레딧은 얼마인가")
    L.append("")
    L.append("지금 값(AI 채점 = 5크레딧)을 그대로 두고 거꾸로 풀면:")
    L.append("")
    L.append(f"    1크레딧 = {top * KRW / 5:.2f}원어치 (가장 비싼 호출 기준)")
    day = 25 + 15 + 5          # 복습 + 세트 + 출석
    L.append(f"    하루 열심히 = 복습 25 + 세트 15 + 출석 5 = {day}크레딧")
    ...
    L.append("- **발음 판정과 AI 대화는 크레딧을 안 깎는다.** 이 둘이 실제로는 횟수가 가장 많다. "
