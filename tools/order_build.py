#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""⚠️ 2026-08-30에 한 번 돌려서 data/order.json을 만든 **일회성 이전 스크립트다.
   지금 다시 돌리면 안 된다** — 그 뒤로 order.json은 손으로(그리고 vocab_audit.py로)
   따로 검수·수정해 왔다(카테고리 재배치·중복 제거·발음 수정 등, 2026-09-09).
   다시 돌리면 그 검수 결과가 전부 사라지고 이 시점의 옛 규칙으로 되돌아간다.
